...
var SeasonAdminApp = React.createClass({
    ...
    onChange: function (e) {
        e.preventDefault();
        if (e.target.value == '-1') {
            this.history.pushState(null, '/app/admin/seasons');
            return;
        }
        var season = null;
    ...
    render: function () {
        ...
        if (this.props.location.query.submitted == 'true') {
            var q = this.props.location.query;
            q.submitted = 'false';
            q = {season: Qs.stringify(selectedSeason)};
            setTimeout(function() {
                this.history.pushState(null, '/app/admin/seasons',q);
                Util.getData('/api/season', function (d) {
                ...
                return (
                    <div className="alert alert-success" role="alert">
                        {'Modified Season! ' + selectedSeason.name}
                    </div>
                );
        }
        return (
            <div id="season-admin-app">
                {select}
                <SeasonModifyApp  season={selectedSeason == undefined ? {} : selectedSeason} divisions={this.state.divisions} />
            </div>
        );
    }
});
...
var SeasonModifyApp = React.createClass({
    ...
    handleSubmit: function(type) {
        ...
        Util.sendData(u,'/api/season/admin/' + type,function(d){
            ...
            this.history.pushState(null, '/app/admin/seasons', query);
        }.bind(this));
    },
